...
class SinusoidalEmbedding(nn.Module):
    ...
    def __init__(self, hidden_dim: int, max_positions: int) -> None:
        super().__init__()
        self.hidden_dim = hidden_dim
        self.max_positions = max_positions
        pos_info = compute_sinusoidal_vectors(max_positions, hidden_dim)
...
def compute_sinusoidal_vectors(max_positions: int, hidden_dim: int) -> T.Tensor:
    """Computes constant vector using sinusoidal wave functions"""
    pos_vectors = T.zeros(max_positions, hidden_dim)
    positions = T.arange(0, max_positions, dtype=T.float).unsqueeze(1)
    mul_coeff = T.exp(
        T.arange(0, hidden_dim, 2) * (-1 * math.log(FREQUENCY) / hidden_dim)
    )
