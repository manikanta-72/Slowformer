...
class EncoderBlock(nn.Module):
    ...
    def forward(self, inputs: T.Tensor, mask: T.Tensor | None) -> T.Tensor:
        ...
        inputs_attn_res = F.layer_norm(inputs + inputs_attn, normalized_shape=(self.d_model,))

        inputs_attn_res_ffn = self.feed_forward(inputs_attn_res)
        inputs_attn_res_ffn_res = F.layer_norm(inputs_attn_res + inputs_attn_res_ffn, normalized_shape=(self.d_model,))
