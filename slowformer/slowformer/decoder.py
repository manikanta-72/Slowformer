...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, dropout_ratio: float) -> None:
        super().__init__()
    ...
    def forward(self, inputs: T.Tensor, encoder_state: T.Tensor, mask: T.Tensor | None) -> T.Tensor:
        ...
        inputs_self_res = self.dropout(F.layer_norm(inputs_self + inputs, normalized_shape=(self.d_model,)))

        inputs_self_res_cross = self.multi_head_cross_attn(encoder_state, encoder_state, inputs_self_res, mask)
        inputs_self_res_cross_res = self.dropout(F.layer_norm(inputs_self_res_cross + inputs_self_res, normalized_shape=(self.d_model,)))

        inputs_self_res_cross_res_ffn = self.feed_forward(inputs_self_res_cross_res)
        inputs_self_res_cross_res_ffn_res = self.dropout(F.layer_norm(inputs_self_res_cross_res_ffn, normalized_shape=(self.d_model,)))
