...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model:int, max_positions: int, n_heads: int, num_encoder_blocks: int, num_decoder_blocks: int, vocab_size: int, dropout_ratio: float = 0.0) -> None:
        ...
        self.encoder = nn.Sequential(*[EncoderBlock(d_model, n_heads) for _ in range(num_encoder_blocks)])
        self.decoder = nn.Sequential(*[DecoderBlock(d_model, n_heads) for _ in range(num_decoder_blocks)])
    ...
    def forward(self, inputs: T.Tensor, labels: T.Tensor, input_mask: T.Tensor | None = None, labels_mask: T.Tensor | None = None) -> T.Tensor:
        ...
        input_embds = self.dropout(self.embedder(inputs))
        label_embds = self.dropout(self.embedder(labels))
